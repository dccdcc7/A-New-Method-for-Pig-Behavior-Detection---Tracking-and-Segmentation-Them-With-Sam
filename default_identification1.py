...
def show_box(frame,id,box,ax):
    ...
    # 使用转换后的数据坐标添加文字
    img_path = './img10/img10/' + r'{}/'.format(id) + r'{}.png'.format(frame)
    print(img_path)
    id = "id: " + str(id)
    class_output = Predict(img_path)
    #ax.text(x0+w-30, y0-5, class_output, fontdict={'fontsize': 8, 'fontweight': 'light', 'color': 'yellow'})
...
def save_masks(input_boxes,box2,masks):
    for i, mask in enumerate(masks):
        ...
        with open(txt_output_path, 'a') as file:
            ...
            # 写入换行符，以便开始新的一行
        plt.axis('off')
        plt.savefig(mask_output_path, dpi=96, bbox_inches='tight', pad_inches=0)
        plt.close()
...
def process_image_by_multianchor(box2,box1,image_path, output_path, dpi=96):
    image = cv2.imread(image_path)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    predictor.set_image(image)
    for item in box1:
...
def by_mulanchor():
    input_folder = "./img10/img1/"
    output_folder = "img10/img10"
    os.makedirs(output_folder, exist_ok=True)
    i = 1
    image_extension = ('.jpg')
    num=0
...
def img2video(folder_path,output_video_path):
    ...
    print(images)
    # 读取第一张图片来获取视频的帧率和尺寸
    frame = cv2.imread(folder_path+"/1.png")
    ...
    for i in range(len(images)+1):
        path_to_image = folder_path + r"/{}.png".format(i+1)
